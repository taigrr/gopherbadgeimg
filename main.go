...
func run(args []string) error {
	...
	if _, err := os.Stat(infile); err != nil {
		return fmt.Errorf("could not stat %v: %w", infile, err)
	}
	...
	err = WriteToGoFile(fmt.Sprintf("%s-generated.go", command), command, imgBits)
	if err != nil {
		return fmt.Errorf("error writing go file: %w", err)
	}
	err = WriteToBinFile(fmt.Sprintf("%s.bin", command), imgBits)
	if err != nil {
		return fmt.Errorf("error writing bin file: %w", err)
	}
...
// WriteToBinFile creates a go:embed-able file containing the image data.
//
...
func WriteToBinFile(filename string, imageBits []byte) error {
	return os.WriteFile(filename, imageBits, 0644)
}

// WriteToGoFile creates a Go file with the bytes hardcoded into a variable at build time.
func WriteToGoFile(filename, variableName string, imageBits []byte) error {
	var output bytes.Buffer
	output.WriteString("// Code generated by gopherbadgeimg DO NOT EDIT.\n\npackage main\n\nvar ")
	...
	for i, b := range imageBits {
		...
		output.WriteString(bStr)
	}
	output.WriteString("\n}\n")

	return os.WriteFile(filename, output.Bytes(), 0644)
}
